...
Contains four loss components (all from QD-DETR):
  ...
  4. loss_entailment (optional) – hyperbolic entailment loss from HLFormer
...
import Models.HLFormer.lorentz as L   # hyperbolic ops from HLFormer
...
class VMRSetCriterion(nn.Module):
    """Compute all losses for the HLFormer-VMR model.
    ...
    Process:
        ...
        5. Optionally compute hyperbolic entailment loss (loss_pop).
    ...
    Args:
        ...
        use_hyperbolic:   if True, also compute the entailment loss from HLFormer
        loss_pop_coef:    weight for the hyperbolic entailment loss
    """
    ...
    def __init__(self, matcher, weight_dict, eos_coef=0.1,
        ...
        self.loss_pop_coef   = loss_pop_coef
    ...
    # ------------------------------------------------------------------
    # Hyperbolic entailment loss (HLFormer contribution)
    # ------------------------------------------------------------------

    # Margin added to the entailment hinge to prevent trivial saturation.
    # Without a positive margin the hinge fires zero as soon as text is
    # anywhere inside the cone, which happens from epoch ~4 and never recovers.
    ENTAIL_MARGIN = 0.05  # v31: was 0.2; reduced to prevent early hinge saturation

    def loss_entailment(self, outputs, targets, indices):
        """Hyperbolic entailment loss (extended from HLFormer loss_pop).

        Three components:
          1. Global positive:  matched (vid, txt) → text inside video cone
          2. Global negative:  hard negative (vid, txt') → txt' outside video cone
             Uses hardest batch negatives (minimum hyperbolic distance) instead of
             trivial torch.roll, forcing the model to push apart similar pairs.
          3. Span-level:  for each Hungarian-matched query, its span feature should
             entail the text query → angle(span, txt) < aperture(span) + margin.
             This provides direct temporal precision signal for R1@0.7.

        Requires outputs:
            "hyp_vid_feat":  (B, D)    – global video rep on hyperboloid
            "hyp_txt_feat":  (B, D)    – global text rep on hyperboloid
            "hyp_span_feat": (B, Q, D) – per-query span rep on hyperboloid
            "_curv": scalar curvature
        """
        if "hyp_vid_feat" not in outputs or outputs["hyp_vid_feat"] is None:
            return {"loss_entailment": torch.tensor(0.0)}

        _curv   = outputs["_curv"]
        vid_hyp = outputs["hyp_vid_feat"]   # (B, D)
        txt_hyp = outputs["hyp_txt_feat"]   # (B, D)

        # ---- 1. Global positive entailment ----
        angle    = L.oxy_angle(vid_hyp, txt_hyp, _curv)
        aperture = L.half_aperture(vid_hyp, _curv)
        pos_loss = torch.clamp(angle - aperture + self.ENTAIL_MARGIN, min=0.0).mean()

        # ---- 2. Hard negative entailment ----
        # Select the hardest negatives (closest wrong text) per video in batch.
        # Much stronger training signal than trivial cyclic roll.
        with torch.no_grad():
            pair_d = L.pairwise_dist(vid_hyp, txt_hyp, _curv)  # (B, B)
            pair_d.fill_diagonal_(float("inf"))
            hard_neg_idx = pair_d.argmin(dim=1)                 # (B,)
        neg_txt   = txt_hyp[hard_neg_idx]
        neg_angle = L.oxy_angle(vid_hyp, neg_txt, _curv)
        neg_loss  = torch.clamp(aperture - neg_angle + self.ENTAIL_MARGIN, min=0.0).mean()

        loss_global = pos_loss + neg_loss

        # ---- 3. Span-level entailment ----
        # For each Hungarian-matched query, the span's hyperbolic representation
        # should entail the text query (span is more specific than global video).
        # Gradient flows directly into temporal boundary predictions.
        loss_span_entail = torch.tensor(0.0, device=vid_hyp.device)
        if ("hyp_span_feat" in outputs and outputs["hyp_span_feat"] is not None
                and indices is not None):
            hyp_span = outputs["hyp_span_feat"]   # (B, Q, D)
            span_terms = []
            for b_i, (src_idx, _) in enumerate(indices):
                if len(src_idx) == 0:
                    continue
                matched_span = hyp_span[b_i, src_idx]                              # (#m, D)
                matched_txt  = txt_hyp[b_i].unsqueeze(0).expand(len(src_idx), -1)  # (#m, D)
                sp_angle    = L.oxy_angle(matched_span, matched_txt, _curv)
                sp_aperture = L.half_aperture(matched_span, _curv)
                span_terms.append(
                    torch.clamp(sp_angle - sp_aperture + self.ENTAIL_MARGIN, min=0.0).mean()
                )
            if span_terms:
                loss_span_entail = torch.stack(span_terms).mean()

        loss_pop = loss_global + loss_span_entail
        return {"loss_entailment": self.loss_pop_coef * loss_pop}

    def loss_hyp_saliency(self, outputs, targets, indices):
        """Hyperbolic saliency ranking loss.

        Uses the Lorentzian frame-text similarity (hyp_saliency) as an
        auxiliary saliency channel. Enforces margin ranking:
            hyp_saliency[pos_clip] > hyp_saliency[neg_clip] + saliency_margin

        This gives the model a hyperbolic-geometry-aware gradient for
        which frames are semantically closest to the text query.

        Requires outputs:
            "hyp_saliency": (B, L_v) – negative Lorentzian inner product
                per frame (higher = closer on hyperboloid = more relevant)
        Requires targets:
            "saliency_pos_labels": (B, n_pairs) – indices of high-saliency clips
            "saliency_neg_labels": (B, n_pairs) – indices of low-saliency clips
        """
        if ("hyp_saliency" not in outputs or outputs["hyp_saliency"] is None
                or "saliency_pos_labels" not in targets):
            return {"loss_hyp_saliency": torch.tensor(0.0)}

        hyp_sal  = outputs["hyp_saliency"]              # (B, L_v)
        pos_idx  = targets["saliency_pos_labels"]        # (B, n_pairs)
        neg_idx  = targets["saliency_neg_labels"]        # (B, n_pairs)
        n_pairs  = pos_idx.shape[1]
        bidx     = torch.arange(len(hyp_sal), device=hyp_sal.device)

        pos_sc = torch.stack(
            [hyp_sal[bidx, pos_idx[:, c]] for c in range(n_pairs)], dim=1)  # (B, n)
        neg_sc = torch.stack(
            [hyp_sal[bidx, neg_idx[:, c]] for c in range(n_pairs)], dim=1)  # (B, n)
        loss = torch.clamp(self.saliency_margin + neg_sc - pos_sc, min=0.0)
        return {"loss_hyp_saliency": loss.mean()}
    ...
    def forward(self, outputs, targets):
        ...
        if self.use_hyperbolic:
            losses.update(self.loss_entailment(outputs_no_aux, targets, indices))
            if outputs_no_aux.get("hyp_saliency") is not None:
                losses.update(self.loss_hyp_saliency(outputs_no_aux, targets, indices))
...
def build_criterion(cfg):
    ...
    Required config keys:
        ...
        use_hyperbolic (optional, default False)
        loss_pop_coef  (optional, default 1e-3)
    ...
    if cfg.get("use_hyperbolic", False):
        weight_dict["loss_entailment"]    = 1.0   # coef already applied inside loss_entailment()
        weight_dict["loss_hyp_saliency"]  = cfg.get("hyp_saliency_coef", 0.5)
    ...
    criterion = VMRSetCriterion(
        ...
        loss_pop_coef  = cfg.get("loss_pop_coef", 1e-3),
