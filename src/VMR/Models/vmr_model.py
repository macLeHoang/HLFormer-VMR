...
class HLFormer_VMR(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.curv = nn.Parameter(
            torch.tensor(float(config.curv_init)).log(),
            requires_grad=config.learn_curv)
        self._curv_minmax = {
    ...
    def forward(self, src_vid, src_vid_mask, src_txt, src_txt_mask):
        ...
        # ---- 8. Hyperbolic entailment (optional) ---------------------
        hyp_vid_feat = hyp_txt_feat = hyp_span_feat = hyp_saliency = _curv = None
        if self.config.use_hyperbolic:
            self.curv.data          = torch.clamp(self.curv.data, **self._curv_minmax)
            # video_alpha / textual_alpha are constrained to <= 0 via gradient hooks
            # registered in _init_weights — no .data clamp needed here.
            _curv = self.curv.exp()

            # Global video representation (mask padding before softmax)
            v_score = self.video_pool(vid_feat).masked_fill(
                src_vid_mask.unsqueeze(-1) == 0, -1e4)
            v_w  = F.softmax(v_score, dim=1)
            v_g  = (vid_feat * v_w).sum(1) * self.video_alpha.exp()
            hyp_vid_feat = L.exp_map0(v_g, _curv)

            # Global text representation
            t_score = self.query_pool(txt_feat).masked_fill(
                src_txt_mask.unsqueeze(-1) == 0, -1e4)
            t_w  = F.softmax(t_score, dim=1)
            t_g  = (txt_feat * t_w).sum(1) * self.textual_alpha.exp()
            hyp_txt_feat = L.exp_map0(t_g, _curv)

            # Span-level hyperbolic features: soft-pool vid_feat within each
            # predicted span then project to hyperboloid.
            # pred_spans: (B, Q, 2) as (cx, w) normalized [0,1]
            _B, _L = vid_feat.shape[:2]
            span_s = (pred_spans[..., 0] - pred_spans[..., 1] / 2).clamp(0., 1.)  # (B, Q)
            span_e = (pred_spans[..., 0] + pred_spans[..., 1] / 2).clamp(0., 1.)  # (B, Q)
            t_pos  = torch.linspace(0., 1., _L, device=vid_feat.device)            # (L,)
            tau    = 0.02   # sigmoid sharpness ≈ 2-3 frames in a 128-frame video
            mask_s = torch.sigmoid((t_pos[None, :, None] - span_s[:, None, :]) / tau)  # (B,L,Q)
            mask_e = torch.sigmoid((span_e[:, None, :] - t_pos[None, :, None]) / tau)  # (B,L,Q)
            soft_mask = mask_s * mask_e                                             # (B, L, Q)
            soft_mask = soft_mask * src_vid_mask[:, :, None].float()
            soft_mask = soft_mask / (soft_mask.sum(1, keepdim=True).clamp(min=1e-8))
            span_feat = torch.einsum('blq,blh->bqh', soft_mask, vid_feat)          # (B, Q, H)
            _B2, _Q, _H = span_feat.shape
            hyp_span_feat = L.exp_map0(
                (span_feat * self.video_alpha.exp()).reshape(_B2 * _Q, _H), _curv
            ).reshape(_B2, _Q, _H)                                                 # (B, Q, H)

            # Hyperbolic saliency: negated Lorentzian inner product between
            # each (scaled) frame feature and the global text feature.
            # The Lorentzian inner product ⟨x,y⟩_L = x₀y₀ - Σxᵢyᵢ is
            # proportional to hyperbolic distance (larger = MORE distant).
            # Negating it gives a similarity score where larger = CLOSER,
            # which is the correct direction for loss_hyp_saliency's margin
            # ranking (pos_clip_score > neg_clip_score).
            _v      = vid_feat * self.video_alpha.exp()                            # (B, L, H)
            _t_exp  = t_g.unsqueeze(1).expand_as(_v)                              # (B, L, H)
            _v_time = torch.sqrt(1.0 / _curv + (_v ** 2).sum(-1))                 # (B, L)
            _t_time = torch.sqrt(1.0 / _curv + (_t_exp ** 2).sum(-1))             # (B, L)
            hyp_saliency = -(
                _v_time * _t_time - (_v * _t_exp).sum(-1)                         # (B, L)
            ) * src_vid_mask.float()
        ...
        out = {
            ...
            "hyp_vid_feat":        hyp_vid_feat,
            "hyp_txt_feat":        hyp_txt_feat,
            "hyp_span_feat":       hyp_span_feat,   # (B, Q, H) – span-level on hyperboloid
            "hyp_saliency":        hyp_saliency,    # (B, L_v)  – Lorentzian frame-text sim
            "_curv":               _curv,
        }
