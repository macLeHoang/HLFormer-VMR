...
def apply_loss_schedule(epoch, criterion, cfg, logger=None):
    ...
    key_map = {
        ...
        "hyp_saliency_coef":          "loss_hyp_saliency",
        "lw_saliency":                "loss_saliency",
        "label_loss_coef":            "loss_label",
    }

    for cfg_key, new_val in current_phase.items():
        if cfg_key in ("set_cost_span", "set_cost_giou", "set_cost_class", "loss_pop_coef"):
    ...
    # Entailment coef — applied inside loss_entailment(), not via weight_dict
    if "loss_pop_coef" in current_phase and hasattr(criterion, "loss_pop_coef"):
        criterion.loss_pop_coef = current_phase["loss_pop_coef"]
